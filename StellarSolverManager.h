#ifndef  __STELLAR_SOLVER_MANAGER
#define __STELLAR_SOLVER_MANAGER
...
public:
    ...
    void startBatchSolving() {
        if (m_jobQueue.isEmpty()) {
            qDebug() << "No jobs to process!";
            QCoreApplication::quit();
            return;
        }

	m_totalJobs = m_jobQueue.size();
        qDebug() << "=== Parallel Plate Solving ===";
        qDebug() << "Total jobs: " << m_totalJobs;
...
void onSolverFinished() {
    ...
    if (solver->solvingDone() && solver->hasWCSData()) {
        ...
        if (m_useXISF) {
            ...
            if (writeWCSToXISF(inputFile, outputFile, solution)) {
                qDebug() << "    XISF written to: " << QFileInfo(outputFile).fileName().toStdString();
            } else {
                qDebug() << "    Failed to write XISF file";
            }
        } else {
		...
		qDebug() << "[Job " << job.jobId << "] ✓ SUCCESS: " << baseName.toStdString();
		qDebug() << "    RA: " << job.ra << "°, Dec: " << job.dec << "°";
		qDebug() << "    Pixel scale: " << job.pixelScale << " arcsec/pix";
		qDebug() << "    WCS written to: " << QFileInfo(outputFile).fileName().toStdString();
		qDebug() << "    Total time: "  << job.totalTime << "s";
	    } else {
		qDebug() << "[Job " << job.jobId << "] ⚠ SOLVED but failed to write WCS: " << baseName.toStdString();
        ...
        job.status = "FAILED";
        qDebug() << "[Job " << job.jobId << "] ✗ FAILED: " << QFileInfo(job.filename).baseName().toStdString();
        if (solver->failed()) {
    ...
    // Progress update
    qDebug() << "Progress: " << m_completedJobs << "/" << m_totalJobs << " completed";
...
private:
    ...
    void startSingleJob(SolveJob job) {
        ...
        if (!loadFITSImage(solver, job.filename)) {
            ...
            qDebug() << "[Job "
            << job.jobId << "] ⚬ SKIPPED: "
            << QFileInfo(job.filename).baseName().toStdString()
            << " (no STELLRA/STELLDEC keywords)";
        ...
        qDebug() << "[Job " << job.jobId << "] Starting: " << QFileInfo(job.filename).baseName().toStdString();
	...
	if (hasSTELLRA && hasSTELLDEC && coordinateRA >= 0 && coordinateRA <= 360 && 
		...
		qDebug() << "[SKIP] No valid coordinates in " << QFileInfo(fitsFile).baseName().toStdString()
			  << " (checked STELLRA/STELLDEC and CRVAL1/CRVAL2)";
		return false;
	    }
	}
    ...
    void finishBatch() {
        ...
        qDebug() << "Summary:";
        qDebug() << "  Successful: " << successful << "/" << m_totalJobs;
        qDebug() << "  Failed: " << failed << "/" << m_totalJobs;
        qDebug() << "  Skipped (no coordinates): " << skipped << "/" << m_totalJobs;
        if (successful > 0) {
            qDebug() << "  Average solve time: "
                      << (totalSolveTime / successful) << "s";
            qDebug() << "  Throughput (successful): "
                      << (successful / (totalBatchTime / 1000.0)) << " solves/second";
        }
	...
	// qDebug() << "FileIO: " << message.toStdString();
    }
...
// =============================================================================
	...
	if (fits_open_file(&inputFits, inputPathBytes.data(), READONLY, &status)) {
	    qDebug() << "Failed to open input FITS: " << inputPath.toStdString()
		      << " (status: " << status << ")";
	    return false;
	}
	...
	if (fits_create_file(&outputFits, outputPathBytes.data(), &status)) {
	    qDebug() << "Failed to create output FITS: " << outputPath.toStdString()
		      << " (status: " << status << ")";
	...
	if (fits_copy_file(inputFits, outputFits, 1, 1, 1, &status)) {
	    qDebug() << "Failed to copy FITS file (status: " << status << ")";
	    fits_close_file(inputFits, &status);
	...
	if (fits_get_img_size(outputFits, 2, naxes, &status)) {
	    qDebug() << "Warning: Could not get image dimensions for WCS reference pixel";
	...
	if (status != 0) {
	    qDebug() << "FITS error while writing WCS data (status: " << status << ")";
	    return false;
	}
